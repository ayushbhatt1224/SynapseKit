...
_WRITER_BASE_URL = "https://api.writer.com/v1"


class WriterLLM(BaseLLM):
    """Writer (Palmyra) LLM provider with async streaming.

    Uses the OpenAI-compatible API. Models include ``palmyra-x-004``,
    ``palmyra-x-003-instruct``, ``palmyra-med``, ``palmyra-fin``, etc.
    Auth via ``config.api_key``.
    """

    def __init__(self, config: LLMConfig, base_url: str | None = None) -> None:
        super().__init__(config)
        self._base_url = base_url or _WRITER_BASE_URL
        self._client = None
