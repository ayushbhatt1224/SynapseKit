...
_NOVITA_BASE_URL = "https://api.novita.ai/v3/openai"


class NovitaLLM(BaseLLM):
    """NovitaAI LLM provider with async streaming.

    Uses the OpenAI-compatible API. Supports popular open models including
    Llama, Mistral, Qwen, and more. Auth via ``config.api_key`` or
    ``NOVITA_API_KEY`` env var (handled by the OpenAI client).
    """

    def __init__(self, config: LLMConfig, base_url: str | None = None) -> None:
        super().__init__(config)
        self._base_url = base_url or _NOVITA_BASE_URL
        self._client = None
